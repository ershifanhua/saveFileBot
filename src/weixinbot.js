...
class WeixinBot extends EventEmitter {
  ...
  async init() {
    ...
    if (this.receiver) {
      ...
      this.transporter.sendMail({
        from: `${this.transporter.transporter.options.auth.user}`,
        to: this.receiver,
        subject: '微信登录二维码图片，请尽快登录',
        text: '请尽快扫码登录，1-2分钟后二维码过期！',
        html: `<a href="${qrcodeUrl}">如果图片无法显示请点击该链接</a><br/><img src="${qrcodeUrl}" height="256" width="256" />`,
      }, (e) => {
  ...
  async getFriend(id) {
    const friend = await this.Contacts.findOneAsync({ UserName: id });
